...
class LocalAuthRepositoryTest {
	...
	@Test
	fun `observeSession starts unauthenticated when no session exists`() =
		runTest {
	...
	@Test
	fun `register stores hashed user and authenticates session`() =
		runTest {
	...
	@Test
	fun `login accepts valid credentials and updates session`() =
		runTest {
			val userDao = FakeUserDao()
			val sessionStore = FakeSessionDataStore()
			val repository = createRepository(userDao, sessionStore)
	...
	@Test
	fun `logout clears persisted session`() =
		runTest {
			val sessionStore = FakeSessionDataStore()
...
private class FakeUserDao : UserDao {
	...
	override suspend fun getActiveById(userId: Uuid): UserEntity? =
		users.value[userId]?.takeIf {
			it.isActive
		}
	...
	): Flow<List<UserEntity>> = users.map { map -> map.values.filter { it.role == role } }
